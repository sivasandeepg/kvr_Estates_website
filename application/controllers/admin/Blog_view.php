...
class Blog_view extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->data['add_subject'] = "Blog view";
        $this->data['subject'] = plural($this->data['add_subject']);
        $this->data['primary_table_name'] = "blog_view";
        ...
        $this->data['hide_columns_in_list_view'] = array("updated_at", 'password', 'salt', 'password',
            ...
            'activation_key',
            'token', 'email_verified', 'slug');

        $this->data['hide_columns_in_edit_view'] = array("updated_at", 'slug');
        $this->data['hide_columns_in_add_view'] = array("status", 'slug');
        $this->data['image_fields'] = array('image', 'image_one', 'image_two', 'image_three', 'image_four');
        ...
        $this->data['relation_fields'] = array(
            ...
            'property_type' => array('table_name' => 'property_type', 'display_column_name' => 'name', "condition" => array("")),
            'blog_categories_id' => array('table_name' => 'blog_categories', 'display_column_name' => 'category_name', "condition" => array("")),
        );
        $this->data['multiple_selection_of_options'] = array("property_type");

        $this->data['column_name_display_as'] = array('role_id' => 'User Role', 'time' => 'Date and Time');

        $this->data['unique_fileds'] = array("title",);
    ...
    function index() {
        ...
        foreach ($this->data["list_items"] as $row) {
            $row->points = substr($row->points, 0, 100) . '......';
            $row->blog_view_description_one = substr($row->blog_view_description_one, 0, 60) . '......';
            $row->blog_view_description_two = substr($row->blog_view_description_two, 0, 60) . '......';
        }
